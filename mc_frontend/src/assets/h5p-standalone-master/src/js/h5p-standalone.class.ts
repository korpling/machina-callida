    ...
    async loadDependencies(toFind: string[], alreadyFound: string[]) {
        ...
        newDependencies.forEach((library: any) => {
            ...
            library.dependencies.forEach((dependency) => {
                if (!dependencies.find((foundLibrary) => foundLibrary.libraryPath === dependency) &&
                    !newDependencies.find((foundLibrary) => foundLibrary.libraryPath === dependency)) {
    ...
    sortDependencies(dependencies) {
        ...
        dependencies.forEach(dependency => {
            ...
            if (dependency.preloadedCss) {
                CSSDependencies[dependency.libraryPath] = CSSDependencies[dependency.libraryPath] ?
            ...
            if (dependency.preloadedJs) {
                JSDependencies[dependency.libraryPath] = JSDependencies[dependency.libraryPath] ?
        ...
        dependencySorter.sort().reverse().forEach((dependencyName) => {
        ...
        if (this.mainLibrary.preloadedCss) {
            Array.prototype.push.apply(styles, this.mainLibrary.preloadedCss.map((style: any) =>
                `${this.librariesPath}/${this.mainLibraryPath}/${style.path}`));
        }
        if (this.mainLibrary.preloadedJs) {
            Array.prototype.push.apply(scripts, this.mainLibrary.preloadedJs.map((script: any) =>
                `${this.librariesPath}/${this.mainLibraryPath}/${script.path}`));
        }
